...
#include "../../../static/ostack/path/ostack.h"
...
int main(int argc, char **argv) {
  ...
  for (unsigned int po2 = min_po2; po2 <= max_po2; ++po2) {
    ...
    for (int run = 0; run < num_runs; ++run) {
      ...
      auto ostack_p = std::make_shared<static_path_ostack::OStack>(size);
      auto alloc_done = now();
      alloc_time += alloc_done - start;

      ostack_p->Push(1, enc_key);
      auto insert_done = now();
      auto insert_done_accesses = ostack_p->MemoryAccessCount();
      auto insert_done_bytes = ostack_p->MemoryBytesMovedTotal();
      insert_time += insert_done - start;
      insert_accesses += insert_done_accesses;
      insert_bytes += insert_done_bytes;

      ostack_p->Peek(enc_key);
      auto search_done = now();
      auto search_done_accesses = ostack_p->MemoryAccessCount();
      auto search_done_bytes = ostack_p->MemoryBytesMovedTotal();
      ...
      ostack_p->Pop(enc_key);
      auto delete_done = now();
      auto delete_done_accesses = ostack_p->MemoryAccessCount();
      auto delete_done_bytes = ostack_p->MemoryBytesMovedTotal();
