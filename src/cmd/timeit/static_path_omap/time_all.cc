...
int main(int argc, char **argv) {
  ...
  for (unsigned int po2 = min_po2; po2 <= max_po2; ++po2) {
    ...
    for (int run = 0; run < num_runs; ++run) {
      ...
      auto omap_p = std::make_shared<static_path_omap::OMap>(size);
      auto alloc_done = now();
      alloc_time += alloc_done - start;

      omap_p->Insert(1, 1, enc_key);
      auto insert_done = now();
      auto insert_done_accesses = omap_p->MemoryAccessCount();
      auto insert_done_bytes = omap_p->MemoryBytesMovedTotal();
      insert_time += insert_done - start;
      insert_accesses += insert_done_accesses;
      insert_bytes += insert_done_bytes;

      omap_p->Read(1, enc_key);
      auto search_done = now();
      auto search_done_accesses = omap_p->MemoryAccessCount();
      auto search_done_bytes = omap_p->MemoryBytesMovedTotal();
      ...
      omap_p->ReadAndRemove(1, enc_key);
      auto delete_done = now();
      auto delete_done_accesses = omap_p->MemoryAccessCount();
      auto delete_done_bytes = omap_p->MemoryBytesMovedTotal();
