...
int main(int argc, char **argv) {
  ...
  for (unsigned int po2 = min_po2; po2 <= max_po2; ++po2) {
    ...
    for (int run = 0; run < num_runs; ++run) {
      ...
      auto omap_p = new dynamic_stepping_path_omap::OMap(po2);
      auto omap = *omap_p;
      auto alloc_done = now();
      alloc_time += alloc_done - start;

      omap.Grow(enc_key);
      omap.Insert(1, 1, enc_key);
      auto insert_done = now();
      auto insert_done_accesses = omap.MemoryAccessCount();
      auto insert_done_bytes = omap.MemoryBytesMovedTotal();
      ...
      auto search_done_accesses = omap.MemoryAccessCount();
      auto search_done_bytes = omap.MemoryBytesMovedTotal();
      ...
      auto delete_done_accesses = omap.MemoryAccessCount();
      auto delete_done_bytes = omap.MemoryBytesMovedTotal();
