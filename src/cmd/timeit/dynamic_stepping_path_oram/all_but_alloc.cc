...
int main(int argc, char **argv) {
  ...
  for (unsigned int po2 = min_po2; po2 <= max_po2; ++po2) {
    ...
    for (int run = 0; run < num_runs; ++run) {
      ...
      auto oram_p = new dynamic_stepping_path_oram::ORam<VAL_LEN>(po2);
      auto oram = *oram_p;
      auto alloc_done = now();
      ...
      auto insert_done_accesses = oram.MemoryAccessCount();
      auto insert_done_bytes = oram.MemoryBytesMovedTotal();
      ...
      auto search_done_accesses = oram.MemoryAccessCount();
      auto search_done_bytes = oram.MemoryBytesMovedTotal();
      ...
      auto delete_done_accesses = oram.MemoryAccessCount();
      auto delete_done_bytes = oram.MemoryBytesMovedTotal();
